function initengahan(){
  ...
  async function pertanyaan(){var { isConfirmed: prtanya } = await swals.fire({title: 'Pilii ' + tanya, text: '' + opstanya, imageUrl: '' + fotostiker6.src, showCancelButton: true, confirmButtonText: '' + tompositif, cancelButtonText: '' + tomnegatif,});
    if(prtanya){
  await swalst.fire({title: '' + katatambahan.innerHTML, timer: 2500, imageUrl: '' + stikerditolak.src,});
      vketik8=vketik81;aktipesan8();
    } else {
	await swalst.fire({title: '' + kataditolak.innerHTML, timer: 6500, imageUrl: '' + stikerditolak.src,});
      vketik8="";aktipesan8();
    }
    }